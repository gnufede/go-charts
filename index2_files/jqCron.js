...
var jqCronDefaultSettings = {
	texts: {
		en : {
			empty: gettext('-all-'),
			name_minute: gettext('minute'),
			name_hour: gettext('hour'),
			name_day: gettext('day'),
			name_week: gettext('week'),
			name_month: gettext('month'),
			name_year: gettext('year'),

			text_period: '<span class="jqCron-human-label">'+gettext('Every')+'</span><span class="jqCron-label">'+gettext('Frequency')+'</span> <b />',
			text_mins: gettext('at')+' <b /> '+gettext('minutes past the hour'),
			text_time: '<span class="jqCron-human-label">'+gettext('at')+'</span><span class="jqCron-label">'+gettext('Hour')+'</span> <b /> <span class="jqCron-human-label">:</span><span class="jqCron-label">'+gettext('Minutes')+'</span> <b />',
			text_dow: '<span class="jqCron-human-label">'+gettext('on')+'</span><span class="jqCron-label">'+gettext('Day')+'</span> <b />',
			text_month: '<span class="jqCron-human-label">'+gettext('of')+'</span><span class="jqCron-label">'+gettext('Month')+'</span> <b />',
			text_dom: '<span class="jqCron-human-label">'+gettext('on the')+'</span><span class="jqCron-label">'+gettext('Day')+'</span> <b />',

			error1: gettext('The tag %s is not supported !'),
			error2: gettext('Bad number of elements'),
			error3: gettext('The jquery_element should be set into jqCron settings'),
			error4: gettext('Unrecognized expression'),
			weekdays: [gettext('monday'), gettext('tuesday'), gettext('wednesday'), gettext('thursday'), gettext('friday'), gettext('saturday'), gettext('sunday')],
...
(function($){
	...
	function jqCron(settings) {
		...
		var _$cross       = $('<span class="jqCron-update"><img src="/static/img/update_icon.png" alt="close" /></span>');
		var _selectors    = [];
		...
		this.setCron = function(str) {
			...
			try {
				...
				var items = str.split(' ');
				if (items.length != 5) _self.error('error2');
        ...
        // wraps <b /> into <span class="jqCron-text"></span>
...
(function($){
	function jqCronSelector(_cron, _$block, _multiple){
		...
		// constructor
    // Replaces left <b /> for <span class="jqCron-selector"></span>
    ...
    _$selector.parent().find('.jqCron-text:empty').last().remove();
    _$selector.prev().andSelf().wrapAll('<span class="jqCron-item" />');
